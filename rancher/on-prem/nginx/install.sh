...
if [ "$1" != "wg" ]; then
  if [ -z "$rancher_nginx_ip" ]; then
    echo -en "=====>\n The following internal ip will have to be DNS-mapped to rancher.aptiway.com and iam.aptiway.com.\n"
  ...
  if [ -z "$rancher_nginx_certs" ]; then
    echo -en "=====>\nGive path for SSL Certificate for rancher.aptiway.com (without any whitespaces) : "
    read rancher_nginx_certs
    rancher_nginx_certs=$(sed 's/\//\\\//g' <<< $rancher_nginx_certs)
  fi
  if [ -z "$rancher_nginx_cert_key" ]; then
    echo -en "=====>\nGive path for SSL Certificate Key for rancher.aptiway.com (without any whitespaces) : "
