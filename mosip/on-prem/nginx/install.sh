...
# Usage: ./install.sh.
  ...
  if [ -z "$cluster_nginx_internal_ip" ]; then
    echo -en "=====>\nThe following internal ip will have to be DNS-mapped to all internal domains from you global_configmap.yaml. Ex: api-internal.sandbox.aptiway.com, iam.sandbox.aptiway.com, etc.\n"
  ...
  if [ -z "$cluster_public_domains" ]; then
    echo -en "=====>\nGive list of (comma seperated) publicly exposing domain names (without any whitespaces). Ex: api.sandbox.aptiway.com, prereg.sandbox.aptiway.com, resident.sandbox.aptiway.com, idp.sandbox.aptiway.com etc : "
    read cluster_public_domains
  fi &&
  if [ -z "$cluster_nginx_certs" ]; then
    echo -en "=====>\nGive path for SSL Certificate (fullchain.pem) for sandbox.aptiway.com (without any whitespaces) : Ex: /etc//letsencrypt/live/sandbox.aptiway.com/fullchain.pem"
    read cluster_nginx_certs
    cluster_nginx_certs=$(sed 's/\//\\\//g' <<< $cluster_nginx_certs)
  fi &&
  if [ -z "$cluster_nginx_cert_key" ]; then
    echo -en "=====>\nGive path for SSL Certificate Key (privkey.pem) for sandbox.aptiway.com (without any whitespaces): Ex: /etc/letsencrypt/live/sandbox.aptiway.com/privkey.pem : "
